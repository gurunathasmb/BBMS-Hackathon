...
import Header from './Head'; // Import the Header component
...
const About = () => {
  ...
  useEffect(() => {
    // Fetch content from an API or backend (replace with your actual backend endpoint)
    const fetchContent = async () => {
      try {
        const response = await fetch('/api/about'); // Replace with your actual endpoint
        const data = await response.json();
        setPageConten(data.page_data); // Assuming 'page_data' is the field in the response
      } catch (error) {
